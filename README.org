...
two players are available, a new tournament is started by the GameEngine.

The maximum number of players per tournament is configurable. The default is 9.

When more than 9 players are available, more than one tournament should be started. The tournament
is responsible for partitioning the available players into tournaments.

*** Tournament

A tournament consists of two or more players playing one round per player.

At the completion of a tournament, the tournament should be responsible for publishing winnings.

At the beginning of each round, one player is designated the dealer, the next player the SmallBlind,
and the following the BigBlind. The tournament is reponsible for shifting the players with these
...
They are created in the Player Repository, and they play Tournaments.
